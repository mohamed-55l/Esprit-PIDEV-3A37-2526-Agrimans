...
class LoginController extends AbstractController
{
    ...
    public function index(
        ...
        // 🛑 إذا كان المستخدم ديجا كونكتي، نبعثوه طول حسب الرول متاعو
        ...
        // 📝 معالجة الدخول الكلاسيكي (Email/Password)
    ...
    public function loginFace(Request $request, EntityManagerInterface $em, SessionInterface $session): Response
    {
        ...
        // لوج على المستخدمين اللي عندهم بصمة وجه
        $users = $em->getRepository(User::class)->findAll();
        $bestMatch = null;
        $threshold = 0.55; // دقة المطابقة (كل ما كان أصغر كل ما كان أدق)
        $minDistance = 1.0;
    ...
    public function home(SessionInterface $session): Response {
        ...
        return $this->render('home/index.html.twig', ['user_name' => $session->get('user_name'), 'user_role' => $session->get('user_role')]);
    }

    #[Route('/admin', name: 'admin_dashboard')]
    public function adminDashboard(SessionInterface $session, EntityManagerInterface $em): Response {
        if ($session->get('user_role') !== 'ADMIN') return new Response("Accès refusé");
        return $this->render('admin/dashboard.html.twig', ['users' => $em->getRepository(User::class)->findAll()]);
    }
