...
use App\Entity\User; // ✅ صلحناها للمفرد
...
class AdminController extends AbstractController
{
    ...
    public function dashboard(SessionInterface $session, EntityManagerInterface $em): Response
    {
        ...
        // 2. جلب المستخدمين باستعمال الـ Entity الصحيحة
        $users = $em->getRepository(User::class)->findAll(); // ✅ User عوضا عن Users

        $stats = [
            'total_users' => count($users),
            'total_equipements' => 0, // تنجم تزيد الـ Logic متاعهم بعد
            'total_reviews' => 0,
            'total_products' => 0,
        ];
